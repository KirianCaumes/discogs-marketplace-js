...
Forked by: CAUMES Kirian (https://github.com/KirianCaumes/)
*/
...
util.processBody = function (body, params) {
	...
	pagination = $(".pagination.bottom").get().length > 0 ? util.processPaginationResponse(paginationBody, params) : 0;
	$(".shortcut_navigable").each(function (i, obj) {
		var item = util.processItem(obj);
		item.isUnavailable = $(obj).hasClass('unavailable');
		listing.push(item);
	});
...
util.processItem = function (obj) {
	...
	item.url = $(".item_description_title").attr('href');
	let labels = []
	$(".label_and_cat a[href^='https://www.discogs.com/']").each((i, el) => labels.push(el.children[0].data))
	item.labels = labels.filter(function (x, i, a) { return a.indexOf(x) == i; }).join(', ');
	item.cat = $(".label_and_cat .item_catno").text();
	item.imageUrl = $(".marketplace_image").attr('data-src');
	item.acceptOffer = $(".item_add_to_cart p strong a").text().split("/").length > 1;
	let temp_condition_sleeve = /\(([^)]+)\)/.exec($(".item_sleeve_condition").text());
	item.condition_sleeve = temp_condition_sleeve && temp_condition_sleeve.length ? temp_condition_sleeve[1] : $(".item_sleeve_condition").text();
	let temp_condition_media = /\(([^)]+)\)/.exec($(".item_condition span").eq(2).text().trim());
	item.condition_media = temp_condition_media && temp_condition_media.length ? temp_condition_media[1] : $(".item_condition span").eq(2).text().trim();
	item.seller = $(".seller_info a").eq(0).text();
	item.ships_from = $(".seller_info li").eq(2).text().split(":").length > 0 ? $(".seller_info li").eq(2).text().split(":")[1] : null
	item.price = util.convertDevise($(".price").eq(0).text().replace(/\s+/g, " ").replace(/,/, '.'));
	item.shipping = util.convertDevise($(".item_shipping").first().children().remove().end().text().replace(/\s+/g, " ").replace(/\+/g, "").replace(/,/, '.').trim());
	item.converted_price = util.convertDevise($(".converted_price").first().children().remove().end().text().replace(/\s+/g, " ").replace(/,/, '.'));
...
util.processPaginationResponse = function (obj, params) {
	var $ = cheerio.load(obj);
	var result = new Object();
	var tempPaginationTotal = $(".pagination_total").text().split(' ').filter(x => x);
	var tempString = tempPaginationTotal[tempPaginationTotal.length - 1].trim();
...
util.convertDevise = function (str) {
	if (!str) return str
	var data = {
		"\$US": "USD",
		"£GB": "GBP",
	...
	Object.keys(data).forEach(x => str = str.replace(`${x}`, data[x] ))	
	return str
}
...
util.generateResult = function (params, callback) {
	...
	if (callback) {
		...
		return rp(options)
			.then(data => {
				return util.processBody(data, params)
			})
			.catch(err => {
				throw err
			});
...
util.buildPath = function (params) {
	var path = `https://www.discogs.com/${params.country ? params.country : 'en'}/sell/${params.type == 'user' ? 'mywants' : 'list'}?`;
	...
	if (params.type == "string")
		...
		path = path.concat("artist_id=", params.id);
	else if (params.type == "user")
		path = path.concat("user=", params.user);
