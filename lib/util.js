...
util.processItem = function (obj) {
	...
	item.price = $(".price").eq(0).text().replace(/\s+/g, " ").replace(/,/, '.');
	item.shipping = $(".item_shipping").first().children().remove().end().text().replace(/\s+/g, " ").replace(/\+/g, "").replace(/,/, '.').trim();
	item.converted_price = $(".converted_price").first().children().remove().end().text().replace(/\s+/g, " ").replace(/,/, '.');
